...
export default function ProjectorClient() {
  ...
  const renderContent = () => {
    ...
    if (isOpen) {
      ...
      const alivePlayers = playersData.filter(p => p.status === "alive").sort((a, b) => a.playerId.localeCompare(b.playerId));
      return (
        <div className="flex flex-col items-start w-[95%] z-20 pb-28 pt-6 gap-6 h-full">
           {/* Top Row: Title + Timer */}
           <div className="flex w-full justify-between items-center gap-8 shrink-0">
             <h2 className="text-[3.5vw] font-serif text-white tracking-widest uppercase leading-tight">
               {activeGameId === "C9" && gameState.phase === "active_a" ? "Step 1 \u2014 Create Secret Sequence" :
                activeGameId === "C9" && gameState.phase === "active_b" ? "Step 2 \u2014 Guess Opponent Sequence" :
                gameState.currentRoundTitle || "Submit Your Decision"}
             </h2>
             {timeLeft !== null && (
               <div className={`text-[10vw] font-mono font-bold leading-none tracking-widest drop-shadow-glow-red shrink-0 ${timeLeft <= 10 ? 'text-primary animate-pulse' : 'text-white'}`}>
           ...
           <div className="w-full shrink-0 space-y-2">
             <div className="flex justify-between items-center text-[1.5vw] font-mono uppercase tracking-widest">
               <span className="text-textMuted">{submissionsCount} / {playersAlive} Submitted</span>
               <span className="text-secondary animate-pulse">Submit on your device</span>
             </div>
             <div className="w-full bg-surface border-2 border-border h-5 overflow-hidden">
               <motion.div
           ...
           {/* Player ID Grid — green when submitted */}
           <div className="w-full flex-1 border-2 border-border bg-surface/60 backdrop-blur-sm p-4 overflow-y-auto">
             <div className="flex flex-wrap gap-2 content-start">
               <AnimatePresence>
                 {alivePlayers.map(p => {
                   const submitted = p.currentSubmission !== null;
                   return (
                     <motion.div
                       key={p.id}
                       initial={{ scale: 0.8, opacity: 0 }}
                       animate={{ scale: 1, opacity: 1 }}
                       className={`px-3 py-2 border font-mono text-[1.3vw] font-bold tracking-widest transition-all duration-700 ${
                         submitted
                           ? 'border-secondary bg-secondary/20 text-secondary shadow-glow-gold'
                           : 'border-border/60 bg-background/40 text-textMuted/60'
                       }`}
                     >
                       {p.playerId}
                     </motion.div>
  ...
  return (
    <main className={`h-screen w-screen bg-black overflow-hidden relative cursor-none flex flex-col items-center ${isOffline ? 'border-8 border-yellow-600' : ''}`}>
      {!isFullscreen && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md">
           <button onClick={toggleFullscreen} className="px-12 py-6 border-4 border-white text-white text-[2.5vw] font-serif uppercase tracking-widest hover:bg-white hover:text-black transition-colors">
      ...
      {/* Main content area — fills all space above bottom bar */}
      <div className="relative z-10 flex flex-col items-center justify-center w-full flex-1 overflow-hidden pb-24">
        {renderContent()}
      </div>

      {/* Persistent Bottom Bar */}
      <div className="absolute bottom-0 left-0 right-0 h-20 bg-surface/70 border-t-2 border-border/60 flex justify-between items-center px-10 z-40 backdrop-blur-sm">
         <span className="text-secondary text-[1.8vw] font-serif uppercase tracking-[0.2em] drop-shadow-glow-gold">House of Trials</span>
         <div className="flex flex-col items-center">
           <span className="text-white text-[1.5vw] font-mono uppercase tracking-widest">{gameState.currentRoundTitle || currentSlotConfig?.gameName || ""}</span>
           <span className="text-textMuted text-[1vw] font-mono uppercase tracking-widest mt-0.5">Phase: {gameState.phase}</span>
         </div>
         <div className="flex items-center gap-6">
           <div className="text-right">
             <p className="text-[1vw] text-textMuted font-mono uppercase tracking-widest">Alive</p>
             <p className="text-[1.8vw] text-white font-bold font-mono">{playersAlive}</p>
           </div>
           {submissionsCount > 0 && (
             <div className="text-right">
               <p className="text-[1vw] text-textMuted font-mono uppercase tracking-widest">Submitted</p>
               <p className="text-[1.8vw] text-secondary font-bold font-mono">{submissionsCount}</p>
             </div>
           )}
         </div>
      </div>
