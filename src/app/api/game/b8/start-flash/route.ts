import { NextRequest, NextResponse } from "next/server";
import { db } from "@/lib/firebase";
import { doc, serverTimestamp, collection, getDocs, writeBatch } from "firebase/firestore";

export async function POST(req: NextRequest) {
  try {
    const { slotNumber, config } = await req.json();

    console.log("[b8/start-flash] Starting flash for slot:", slotNumber, "config:", JSON.stringify(config));

    const batch = writeBatch(db);

    const defaultConfig = {
      questionId: null,
      customQuestion: null,
      correctAnswer: "A",
      optionALabel: "Option A",
      optionBLabel: "Option B",
      imageFlashSeconds: 3,
      votingSeconds: 7,
      confidenceEnabled: true,
      confidenceSeconds: 5,
      fakeMajorityEnabled: true,
      fakeMajorityBiasToward: "A",
      fakeMajorityStartPercent: 72,
    };

    const finalConfig = config || defaultConfig;

    batch.update(doc(db, "system", "gameState"), {
      phase: "image_flash",
      b8Config: finalConfig,
      b8Results: null,
      imageFlashStartedAt: serverTimestamp(),
    });

    const playersSnap = await getDocs(collection(db, "players"));
    console.log("[b8/start-flash] Resetting submissions for", playersSnap.size, "players");
    playersSnap.docs.forEach(p => {
      batch.update(p.ref, { currentSubmission: null, submittedAt: null });
    });

    await batch.commit();
    console.log("[b8/start-flash] SUCCESS. Flash started.");

    return NextResponse.json({ success: true, phase: "image_flash", config: finalConfig });
  } catch (error: any) {
    console.error("[b8/start-flash] ERROR:", error.code, error.message, error.stack);
    return NextResponse.json({ error: error.message, code: error.code }, { status: 500 });
  }
}
